...
We all know how Emacs is not just a text editor that one only thinks
of while needing to editing a text file, but instead is a multi-tool
that can be used for (almost) anything and everything one uses a
computer for.

I named this document *World of Emacs* as this tool is surrounded by its
own ecosystem, to the extent that I think naming it something like
*Emacs Universe* would also be reasonable enough. At least to me, the
experiences I've had with this tool is beyond comparison to any other
software tool that I've used on a computer. Though this phenomenon
that I refer to here cannot always be described in words to the
"non-believers", this is something that may just start off as a small
tools in your tool-set, slowly growing in all dimensions, eventually
replacing most other tools around it. This does come from the fact
that Emacs already has a mode to do pretty much anything that you do
in any other software, but Emacs users also often end up creating what
is not there already, or otherwise create their own simplified version
